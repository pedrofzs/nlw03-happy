...
export default function Orphanage() {
  ...
  function handleOpenWhatsApp(){      
    Linking.openURL(`https://api.whatsapp.com/send?phone=${orphanage?.whatsappNumber}`);
  }
      ...
      <View style={styles.imagesContainer}>
        <ScrollView horizontal pagingEnabled>
            {orphanage.images.map(img => {
                return <Image key={img.id} style={styles.image} source={{ uri: img.url }}></Image>
            })}          
        </ScrollView>
      </View>
      <View style={styles.orphanageContainer}>
        <Text style={styles.title}>{orphanage.name}</Text>
        <Text style={styles.description}>{orphanage.about}</Text>      
        <View style={styles.mapContainer}>
          <MapView initialRegion={{ latitude: orphanage.latitude, longitude: orphanage.longitude, latitudeDelta: 0.008, longitudeDelta: 0.008, }} zoomEnabled={false} pitchEnabled={false} scrollEnabled={false} rotateEnabled={false} style={styles.mapStyle}>
            <Marker icon={mapMarkerImg} coordinate={{ latitude: orphanage.latitude, longitude: orphanage.longitude }}></Marker>
          </MapView>
        ...
        <View style={styles.scheduleContainer}>
          ...
          {orphanage.openOnWeekend ? (
                                <View style={[styles.scheduleItem, styles.scheduleItemGreen]}>
                                    ...
                                    <Text style={[styles.scheduleText, styles.scheduleTextGreen]}>Atendemos final de semana</Text>
                                </View>
                                ) : (
